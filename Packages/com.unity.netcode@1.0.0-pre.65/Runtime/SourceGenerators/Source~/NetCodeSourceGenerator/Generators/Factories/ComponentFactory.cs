    ...
    internal class ComponentFactory
    {
        ...
        private static void GenerateComponents(IEnumerable<SyntaxNode> components, CodeGenerator.Context codeGenContext)
        {
            ...
            foreach (var componentCandidate in components)
            {
                ...
                {
                    codeGenContext.diagnostic.LogInfo($"Skipping code-gen for {candidateSymbol.Name} because a component serializer for it already exists");
                    continue;
                }
        ...
        private static void GenerateVariants(IEnumerable<SyntaxNode> variants, CodeGenerator.Context codeGenContext)
        {
            ...
            foreach (var componentCandidate in variants)
            {
                ...
                {
                    codeGenContext.diagnostic.LogInfo($"Skipping code-gen for {variantSymbol.Name} because a variant component serializer for it already exists");
                    continue;
                }
                ...
                codeGenContext.diagnostic.LogInfo($"Generating serializer for variant {variantSymbol.ToDisplayString()} for type {variantTypeInfo.TypeFullName}.");
                codeGenContext.ResetState();
                codeGenContext.generatorName = Roslyn.Extensions.GetTypeNameWithDeclaringTypename(variantSymbol);
                codeGenContext.variantType = Roslyn.Extensions.GetFullTypeName(variantSymbol).Replace('+', '.');

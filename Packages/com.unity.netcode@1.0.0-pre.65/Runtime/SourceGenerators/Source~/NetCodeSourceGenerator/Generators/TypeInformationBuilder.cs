...
namespace Unity.NetCode.Generators
{
    ...
    internal struct TypeInformationBuilder
    {
        ...
        public TypeInformation BuildTypeInformation(ITypeSymbol symbol, GhostComponentAttribute ghostAttribute, GhostField ghostFieldOverride = null)
        {
            ...
            m_Reporter.LogInfo($"Building type info for {symbol}");
            ...
            var typeInfo = new TypeInformation
            {
                ...
                AttributeMask = m_SerializationMode == SerializationMode.Component
                ...
                HasSupportsPrefabOverridesAttribute = Roslyn.Extensions.GetAttribute(symbol, "Unity.NetCode", "SupportsPrefabOverridesAttribute") != null,
                IsTestVariant = false,
            };
        ...
        public TypeInformation BuildVariantTypeInformation(ITypeSymbol variantSymbol, AttributeData variantAttribute, GhostComponentAttribute ghostAttribute)
        {
            ...
            using (new Profiler.Auto("ValidationAndExtraction"))
            {
                ...
                foreach (var member in variantSymbol.GetMembers().OfType<IFieldSymbol>())
                {
                    ...
                    if(originalMember == null || (originalMember as IFieldSymbol)?.Type.Name != member.Type.Name)
                    {
                        hasErrors = true;
                ...
                foreach (var prop in variantSymbol.GetMembers().OfType<IPropertySymbol>())
                {
                    ...
                    if(originalMember == null || (originalMember as IPropertySymbol)?.Type.Name != prop.Type.Name)
                    {
                        hasErrors = true;
            ...
            using (new Profiler.Auto("ParseMembers"))
            {
                ...
                {
                    m_Reporter.LogInfo($"Parsing field {member}");
        ...
        public TypeInformation ParseFieldType(ISymbol member, ITypeSymbol memberType, TypeInformation parent, string fieldPath, int level=1, GhostField ghostFieldOverride = null)
        {
            ...
            if ((typeKind != GenTypeKind.Struct) && (ghostField != null && ghostField.Composite))
            ...
            if (ghostField != null)
            {
                ...
                if (ghostField.Composite) typeInfo.Attribute.composite = ghostField.Composite;
            ...
            typeInfo.Attribute.composite &= (typeInfo.AttributeMask & TypeAttribute.AttributeFlags.Composite) != 0;

            if((typeInfo.AttributeMask & TypeAttribute.AttributeFlags.Quantized) == 0)
                typeInfo.Attribute.quantization = -1;

            //Always reset the composite flag for the primitive types
            if (typeKind != GenTypeKind.Struct && typeInfo.Attribute.composite)
            {
                ...
                typeInfo.Attribute.composite = false;
            }

...
{
    /// <summary>
    /// Finds and calls ICustomBootstrap and manages finding, registering and sorting systems.
    /// Replaces <see cref="DefaultWorldInitialization"/>
    /// </summary>
...
#pragma warning restore 0067 // unused variable
        ...
        static ICustomBootstrap CreateBootStrap()
        {
            ...
            // Get the deepest implementations of ICustomBootstrap
            List<Type> deepestBootstrapImplementations = new List<Type>();
            foreach (var bootType in bootstrapTypes)
            {
                // Ignore the ICustomBootstrap interface
                if (bootType.IsAbstract || bootType.ContainsGenericParameters)
                    continue;
                
                // If type is not parent or child of any type, add it
                // if type is parent of already seen type, don't add it
                // if type is child of already seen type, remote the parent, add the child
                bool isSubType = true;
                foreach (var otherBootType in deepestBootstrapImplementations)
                {
                    // Check if bootType is a sub-type of a previously seen type
                    if (bootType.IsAssignableFrom(otherBootType))
                    {
                        deepestBootstrapImplementations.Remove(otherBootType);
                        break;
                    }
                    if (otherBootType.IsAssignableFrom(bootType))
                    {
                        isSubType = false;
                        break;
                    }
                ...
                if(isSubType)
                    deepestBootstrapImplementations.Add(bootType);
            }
            
            // Split bootstrap types into Unity (built-in) or custom
            List<Type> unityBootstraps = new List<Type>();
            List<Type> customBootstraps = new List<Type>();
            foreach (var bootType in deepestBootstrapImplementations)
            {
                if(bootType.FullName != null && bootType.FullName.Contains("Unity"))
                    unityBootstraps.Add(bootType);
                else
                    customBootstraps.Add(bootType);
            }

            if (unityBootstraps.Count == 0 && customBootstraps.Count == 0)
            {
                Debug.LogError("No ICustomBootstrap found!");
                return null;
            }
            if (customBootstraps.Count > 1)
            {
                Debug.LogWarning($"Multiple custom ICustomBootstrap found! Ignoring {customBootstraps.GetRange(1, customBootstraps.Count - 1)}");
            }
            Type selectedType = null;
            if (customBootstraps.Count == 1)
            {
                selectedType = customBootstraps[0];
            }
            else 
            {
                selectedType = unityBootstraps[0];
            }

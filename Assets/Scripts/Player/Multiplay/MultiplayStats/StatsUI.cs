    ...
    public class StatsUI : MonoBehaviour
    {
        ...
        private static string CreateDisplayString(RTCStatsReport report, RTCStatsReport lastReport)
        {
            ...
            foreach (var stats in report.Stats.Values)
            {
                ...
                {
                    builder.AppendLine($"{remoteOutboundStats.kind} sending remote stream stats");
                    int rttm = ((RTCRemoteInboundRtpStreamStats) stats).roundTripTimeMeasurements;
                    double trtt = ((RTCRemoteInboundRtpStreamStats) stats).totalRoundTripTime;
                    double fl = ((RTCRemoteInboundRtpStreamStats)stats).fractionLost;

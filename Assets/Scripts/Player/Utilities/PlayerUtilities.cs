...
// Taken from NetCode samples
// https://github.com/Unity-Technologies/EntityComponentSystemSamples/tree/master/NetcodeSamples/Assets/Samples/HelloNetcode/2_Intermediate/01_CharacterController
// Stores the impulse to be applied by the character controller body
public struct DeferredPlayerImpulse
{
...
public static class PlayerUtilities
{
    ...
    public struct PlayerAllHitsCollector<T> : ICollector<T> where T : unmanaged, IQueryResult
    {
        ...
        public PlayerAllHitsCollector(int rbIndex, float maxFraction, ref NativeList<T> allHits, PhysicsWorldSingleton physicsWorldSingleton,
        ...
        public PlayerAllHitsCollector(int rbIndex, float maxFraction, ref NativeList<T> allHits, PhysicsWorld world,
            NativeList<T> triggerHits = default)
        {
            MaxFraction = maxFraction;
            AllHits = allHits;
    ...
    public struct PlayerClosestHitCollector<T> : ICollector<T> where T : struct, IQueryResult
    {
        public bool EarlyOutOnFirstHit => false;
        ...
        public PlayerClosestHitCollector(NativeList<SurfaceConstraintInfo> predefinedConstraints, PhysicsWorld world, int rbIndex, float maxFraction)
        {
            MaxFraction = maxFraction;
        ...
        public PlayerClosestHitCollector(NativeList<SurfaceConstraintInfo> predefinedConstraints, PhysicsWorldSingleton world, int rbIndex, float maxFraction)
        {
            MaxFraction = maxFraction;
            m_ClosestHit = default;
            NumHits = 0;
    ...
    public static void CheckSupport(
        in PhysicsWorldSingleton physicsWorldSingleton, ref PhysicsCollider collider, PlayerStepInput stepInput, RigidTransform transform,
        out PlayerSupportState playerState, out float3 surfaceNormal, out float3 surfaceVelocity)
    {
        ...
        PlayerAllHitsCollector<ColliderCastHit> castHitsCollector = new PlayerAllHitsCollector<ColliderCastHit>(
        ...
        {
            playerState = PlayerSupportState.Unsupported;
            return;
        }
        ...
        // Check support state
        {
            ...
            else if (math.lengthsq(outVelocity) < k_SimplexSolverEpsilonSq && numSupportingPlanes > 0)
            {
                ...
                playerState = PlayerSupportState.Supported;
            }
            else
            {
                // Check if sliding
                ...
                {
                    playerState = PlayerSupportState.Sliding;
                }
    ...
    public static void CollideAndIntegrate(
        PlayerStepInput stepInput, float characterMass, bool affectBodies, ref PhysicsCollider collider,
        ref RigidTransform transform, ref float3 linearVelocity, ref NativeStream.Writer deferredImpulseWriter)
    {
        ...
        for (int i = 0; i < stepInput.MaxIterations && remainingTime > timeEpsilon; i++)
        {
            ...
            // Do a collider cast
            {
                ...
                NativeList<ColliderCastHit> castHits = new NativeList<ColliderCastHit>(k_DefaultQueryHitsCapacity, Allocator.Temp);
                PlayerAllHitsCollector<ColliderCastHit> collector = new PlayerAllHitsCollector<ColliderCastHit>(
            ...
            // but only fix up penetrating hits
            {
                ...
                PlayerAllHitsCollector<DistanceHit> distanceHitsCollector = new PlayerAllHitsCollector<DistanceHit>(
            ...
            if (math.lengthsq(newDisplacement) > k_SimplexSolverEpsilon)
            {
                ...
                var newCollector = new PlayerClosestHitCollector<ColliderCastHit>(constraints, stepInput.PhysicsWorldSingleton, stepInput.RigidBodyIndex, 1.0f);
    ...
    private static void CalculateAndStoreDeferredImpulsesAndCollisionEvents(
        PlayerStepInput stepInput, bool affectBodies, float characterMass,

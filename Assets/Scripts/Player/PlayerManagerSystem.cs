...
namespace Opencraft.Player
{
    ...
    public partial class PlayerManagerSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            {
                var networkId = networkIdFromEntity[requestSource.SourceConnection];
                var found = false;
                ...
                {
                    var playerComponent = playerData[i];
                    if (playerComponent.Username == reqSpawn.Username)
                    {
                        ...
                        Debug.Log($"Linking user '{reqSpawn.Username}@conn{networkId.Value}' to existing player!");
                ...
                if (!found)
                {
                    Debug.Log($"'Spawning a Ghost '{prefabName}' for user '{reqSpawn.Username}' on connection '{networkId.Value}'!");
                    ...
                    commandBuffer.SetComponent(player, LocalTransform.FromPosition(playerSpawn.location));
                }

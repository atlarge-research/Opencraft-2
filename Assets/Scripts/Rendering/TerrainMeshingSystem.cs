...
public partial class TerrainMeshingSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        NativeArray<TerrainArea> terrainChunks = _terrainAreaQuery.ToComponentDataArray<TerrainArea>(Allocator.TempJob);
        ...
        MeshTerrainChunkJob meshJob = new MeshTerrainChunkJob
        {
            ...
            blocksPerSideCubed = terrainSpawner.blocksPerSide * terrainSpawner.blocksPerSide *terrainSpawner.blocksPerSide,
        ...
        // Mark that these areas have been meshed
        EntityManager.SetComponentEnabled<Remesh>(_terrainAreaQuery, false);
    }
}


// Meshing algorithm adapted by https://vercidium.com/blog/voxel-world-optimisations/
...
public partial struct MeshTerrainChunkJob : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        int access;
        int numFaces = 0;
        //i, j and k refer to chunk-relative positions (range 0-blocksPerSide-1)
    ...
    private bool VisibleFaceXN(int i, int j, int k, ref DynamicBuffer<TerrainBlocks> blocks, ref DynamicBuffer<TerrainBlocks> neighborXN)
    {
    ...
    private bool VisibleFaceXP(int i, int j, int k, ref DynamicBuffer<TerrainBlocks> blocks, ref DynamicBuffer<TerrainBlocks> neighborXP)
    {
        if (i >= blocksPerSide)
        {
    ...
    private bool VisibleFaceZN(int i, int j, int k, ref DynamicBuffer<TerrainBlocks> blocks, ref DynamicBuffer<TerrainBlocks> neighborZN)
    {
        if (k < 0)
        {
    ...
    private bool VisibleFaceZP(int i, int j, int k, ref DynamicBuffer<TerrainBlocks> blocks, ref DynamicBuffer<TerrainBlocks> neighborZP)
    {
        if (k >= blocksPerSide)
        {
    ...
    private bool VisibleFaceYN(int i, int j, int k, ref DynamicBuffer<TerrainBlocks> blocks, ref DynamicBuffer<TerrainBlocks> neighborYN)
    {
        if (j < 0)
        {
    ...
    private bool VisibleFaceYP(int i, int j, int k, ref DynamicBuffer<TerrainBlocks> blocks, ref DynamicBuffer<TerrainBlocks> neighborYP)
    {
        if (j >= blocksPerSide)
        {
    ...
    /*
     ...
     todo- of 3 bits. 
     */
    private void AppendQuad(ref NativeArray<Vector3> verts, ref NativeArray<Vector3> norms, ref NativeArray<ushort> indices,

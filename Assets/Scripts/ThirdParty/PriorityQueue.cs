...
// https://github.com/BlueRaja/High-Speed-Priority-Queue-for-C-Sharp/tree/master
namespace Priority_Queue
{
    ...
    public interface IPriorityQueue<TItem, TPriority> : IEnumerable<TItem>
    {
        ...
        /// </summary>
        TItem Dequeue(out TPriority priority);
        ...
        /// </summary>
        bool Contains(TItem node, out TPriority priority);
        ...
        where TItem : GenericPriorityQueueNode<TPriority>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        public bool Contains(TItem node, out TPriority priority)
        {
#if DEBUG
            if(node == null)
            {
...
#endif
            if (_nodes[node.QueueIndex] == node)
            {
                priority = _nodes[node.QueueIndex].Priority;
                return true;
            }
            priority = default;
            return false;
        ...
        public void Enqueue(TItem node, TPriority priority)
        {
            ...
            if (Contains(node, out TPriority prio))
            {
        ...
        public void UpdatePriority(TItem node, TPriority priority)
        {
            ...
            if (!Contains(node, out TPriority _))
            {
    ...
    public class SimplePriorityQueue<TItem, TPriority> : IPriorityQueue<TItem, TPriority>
    {
        ...
        /// O(1)
        /// </summary>
        public bool Contains(TItem item, out TPriority priority)
        {
            lock(_queue)
            {
                bool con = item == null ? _nullNodesCache.Count > 0 : _itemToNodesCache.ContainsKey(item);
                if(!con)
                    priority = default(TPriority);
                else
                {
                    priority = GetExistingNode(item).Priority;
                }
                return con;
            }
        }
        ...
        public TItem Dequeue(out TPriority priority)
        {
            lock(_queue)
            {
                ...
                SimpleNode node =_queue.Dequeue(out TPriority s_priority);
        ...
        public bool TryDequeue(out TItem first)
        {
            if (_queue.Count > 0)
            {
                lock (_queue)
                {
                    if (_queue.Count > 0)
                    {
                        SimpleNode node = _queue.Dequeue(out TPriority priority);
                        first = node.Data;
                        RemoveFromNodeCache(node);
                        return true;
                    }
        ...
        public bool IsValidQueue()
        {
            lock(_queue)
            {
                ...
                foreach (IList<SimpleNode> nodes in _itemToNodesCache.Values)
                {
                    ...
                    {
                        if (!_queue.Contains(node, out TPriority priority))
                        {
                            return false;
                        }
                    }
